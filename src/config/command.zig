//! whostty: the `command` config value (the program a surface runs).
//!
//! Faithful port of ghostty `src/config/command.zig`. A command is either a
//! shell string to be expanded (`shell:`, the default) or a pre-split argv
//! (`direct:`). whostty's PTY currently hard-codes `cmd.exe`; this type is the
//! config-level surface so `command = pwsh -NoLogo` (and `direct:` to skip shell
//! expansion) can be expressed and round-tripped. The ConPTY wiring that
//! consumes it beyond `-e` is tracked separately (gap-audit / #55).
...
/// shell expansion, specify a `direct:`-prefix. e.g. `direct:nvim a b c`.
///
...
pub const Command = union(enum) {
    ...
    /// Execute a command directly, e.g. via `exec`. The format here is already
    /// structured to be passed directly to `exec` with index zero being the
    /// command. Index zero may require PATH lookup by the downstream.
    direct: []const [:0]const u8,

    /// Execute a command via shell expansion. This provides the command as a
    /// single string that is expected to be expanded in some way (up to the
    /// downstream).
    shell: [:0]const u8,
    ...
    /// Creates a command as a single string, joining arguments as necessary
    /// with spaces. It is only meant to be human readable.
    pub fn string(
        self: *const Self,
        alloc: Allocator,
    ...
    /// Get an iterator over the arguments array. This may allocate depending
    /// on the active tag of the command.
    pub fn argIterator(
        self: *const Self,
        alloc: Allocator,
