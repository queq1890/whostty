//! whostty: config string-literal parsing.
//!
//! Faithful port of ghostty `src/config/string.zig`. Config values may contain
//! Zig string-literal escape sequences (`\n`, `\t`, `\u{...}`, `\\`, …); this
//! parses them into raw bytes and provides an allocation-free codepoint iterator
//! that decodes escapes on the fly. Used by keys whose values are user-visible
//! text (e.g. window titles, prompts) where escapes must be honored.
...
test "parse: invalid escape is an error" {
    var buf: [8]u8 = undefined;
    try std.testing.expectError(error.InvalidString, parse(&buf, "\\q"));
}
