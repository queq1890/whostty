//! whostty: conditional configuration state.
//!
//! Faithful port of ghostty `src/config/conditional.zig`. Conditionals are based
//! on a static, typed state of the world (the OS desktop theme light/dark, the
//! target OS) instead of a dynamic key-value set. This is what powers
//! `theme = light:rose-pine-dawn dark:rose-pine` and other OS-appearance
//! switching: the loader re-evaluates conditional config when the state changes.
const std = @import("std");
const builtin = @import("builtin");
const Allocator = std.mem.Allocator;

/// Conditionals in whostty configuration are based on a static, typed
...
test "conditional os key" {
    const testing = std.testing;
    const state: State = .{ .os = .windows };
    try testing.expect(state.match(.{
        .key = .os,
        .op = .eq,
        .value = "windows",
