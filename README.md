...
| module | what it is |
|---|---|
| `whostty-engine` | the platform-free engine model (#129/#130): `grid` (cell/layout geometry), `split` (`SplitTree`/`TabList`), `mouse` (VT mouse-report encoding), `scroll`, `frame`, `host` (the apprt-free `Host` vtable, #132), `surface` (the `Geometry` cell-metrics/resize/reflow model, #133), and `cwd` (the unified per-pane working-directory store, #134) — zero Win32 / ConPTY / WGL dependency. |
