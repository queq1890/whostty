...
| App (Windows) | `src/apprt/gtk/App.zig` | `src/apprt/win32/App.zig` | template | slice-0 + SGR cells, config-driven colors/size (#12/#17); **frame pacing** (`src/engine/frame.zig`): redraw only when the terminal's dirty generation, the (blinking) cursor phase, focus, or a UI event changed — otherwise the loop sleeps via `MsgWaitForMultipleObjects` and the reader thread wakes it with `PostMessage` on new output (#72) | links; frame-pacing logic host-tested |
| Surface | `src/Surface.zig` | `src/Surface.zig` + `src/engine/grid.zig` | port | resize done + window padding (`window-padding-x/y/-balance`): `layout()` reserves padding on each side, fits the grid, and offsets the origin (renderer + mouse both honor it); balance centers the grid (#71). Per-cell selection reverse-video done (#51, `buildQuads`); custom GLSL shaders deferred (host). **Engine split (#129)**: the pure grid/cell/layout geometry (`GridSize`/`Cell`/`Padding`/`Layout`/`layout`/`cellFromPixels`/`gridFromPixels` + tests) moved to the platform-free `src/engine/grid.zig`; `Surface.zig` re-exports it and keeps the ConPTY-bound `Surface` struct | done (host: layout/origin); GL (offscreen-proof: pad shifts glyph) |
| Engine layer (E0) | — (whostty-specific) | `src/engine/` (`engine.zig` barrel, `grid.zig`, `SplitTree.zig`, `mouse.zig`, `scroll.zig`, `frame.zig`) | template | **platform-free terminal-engine boundary (#129, epic E0 #141)**: the grid/cell/layout geometry, SplitTree/TabList model, and mouse/scroll/frame logic relocated into one namespace with ZERO Windows / ConPTY / WGL dependency, the importable layer whomux builds on. `zig build engine-test` compiles + unit-tests it for the Linux CI host (a non-Windows target), so CI goes red if a Win32 import leaks in; `engine-linux` is an explicit cross-target link-check | done (host: all engine units on the non-Windows boundary target) |
...
| Scrollback scroll | `src/Surface.zig` (scroll); storage in VT core | `src/engine/scroll.zig` (wheel→rows, page, scrollbar thumb) + `termio` scroll, viewport in `ghostty-vt` `PageList` | port + dependency | wheel + page scroll, scroll-to-bottom, scrollbar geometry (#16); **visible scrollbar** rendered as an overlay thumb on the right edge from `PageList.scrollbar()` row counts, shown only with scrollback (#73, drag-thumb interaction deferred) | done (host: wheel/page/scrollbar + `Termio.scrollbar`); GL (offscreen-proof: thumb tracks viewport) |
| Renderer registry | `src/renderer.zig` | `src/renderer.zig` | template | backend abstraction + `renderer` config selector; **both backends implemented** — OpenGL (default) and **Direct3D 11** (#15/#43), selected via `renderer = direct3d`; the app holds the chosen backend in a tagged union dispatched with `inline else` (no hot-path vtable) | done (host: enum guard) |
...
| Renderer (Direct3D 11) | `src/renderer/OpenGL.zig` (contract) | `src/renderer/Direct3D11.zig` + `src/os/d3d11.zig` (COM/DXGI/d3dcompiler bindings) | port | **native D3D11 backend (#15/#43)** mirroring the OpenGL pass: `D3D11CreateDeviceAndSwapChain` (HARDWARE→WARP fallback) for the HWND, runtime-compiled HLSL (`D3DCompile` via `d3dcompiler_47`) vertex+pixel shaders with the same mode switch (alpha glyph / solid / color glyph), a dynamic vertex buffer (grow + `MAP_WRITE_DISCARD`), an R8 glyph atlas + RGBA color atlas as `Texture2D`+SRV (point/clamp sampler), straight-alpha blend, and `Present`. Reuses OpenGL's host-tested `Cell`/`SolidRect`/`Quad`/`Vertex`+`pushQuad`/`pushSolid`/`pushColorQuad` (no geometry duplication). `draw` detects a back-buffer size change and `ResizeBuffers` internally; GPU-runtime failures (resize/buffer/map/present) are non-fatal (skip the frame, recover next), so only OOM propagates — matching the GL contract. Window skips the WGL context for the D3D arm (`Window.init(use_gl=false)`); the renderer owns the swap chain (`present`). Full device-removed recreation is a follow-up. The hand-written COM vtables mirror `os/dwrite.zig` | on-device verified (rose-pine theme bg + shell text glyphs + block cursor + maximize/ResizeBuffers, `renderer=direct3d`) |
...
| Config | `src/config/` (Config.zig, file_load, path, theme, conditional, command, formatter, string) + `src/cli/args.zig` (LineIterator) | `src/config.zig` (aggregator) + `src/config/` | port | **Full config system (#49)**: `src/config.zig` is now an aggregator re-exporting `config/Config.zig` (the field surface) + the supporting modules. Field surface broadened well past the #17 subset — fonts (family/bold/italic, thicken, grapheme-width-method, freetype-load-flags packed, adjust-* metric modifiers, font-variation), colors/appearance, window geometry/decoration/save-state, splits, behavior (confirm-close/copy-on-select/clipboard access/mouse-shift-capture/focus-follows-mouse), command (`Command` shell/direct union)/working-directory/shell-integration(+features packed)/env, background image/opacity/blur, custom-shader — parse + store + validate + **formatter (write-back)** for all (round-trip tested); runtime *behavior* for many keys stays delegated (render #65/#66, windowing #68, search #94, shell-int #55, VT #67). **`config-file` recursive includes** with cycle guard + Windows path expansion (`~`/`%VAR%`, `config/path.zig`). **Themes**: `theme` key + `light:…dark:…` conditional (`config/conditional.zig`) resolved against the OS light/dark appearance (`os/windows.zig` `isSystemDarkMode` reads the registry), an embedded built-in theme catalog (`config/themes/`), and user/resources theme dirs; theme is a base layer the user config overrides. Windows default path `%APPDATA%\whostty\config.whostty` (legacy `config`) via `config/file_load.zig`. Earlier: colors/font/cursor/palette/renderer/font-feature/keybind/cursor+selection/cursor-opacity/bold-is-bright/scrollback-limit (#17/#69) | done (host: parse/format round-trip, includes/theme/cycle via tmpDir; OS-appearance + on-disk theme dirs host-only) |
| Surface mgmt (tabs/splits) | `src/apprt/gtk/` (Split/Notebook) | `src/engine/SplitTree.zig` + `apprt/win32/App.zig` (`Pane`/`WinState`) | template | split tree (split/close/resize/equalize/layout/focus-nav/hit-test + `dividers`/`anyLeaf`) + tab list model (#18); **wired into the apprt (#87)**: each pane is an independent terminal (own ConPTY+VT+reader thread+`Surface`, heap-stable), instantiated from the tree; `new_split`/`goto_split`(directional, `focusTarget`)/`close_surface`(sibling collapses; last pane closes the window)/`new_tab`/`next_tab`/`previous_tab`/`goto_tab` dispatched via `performAction`; render composites every pane (per-pane bg/cursor/scrollbar) + split dividers + a tab strip in one GL pass; mouse routed to the pane under the cursor (focus on click, drag owned per-pane), tab-strip click switches tabs; only the focused pane shows a solid cursor (others hollow). IME (#88) and split-resize/equalize keybinds are follow-ups | fresh (host: tree incl. dividers/anyLeaf); on-device verified (split → 3 panes, close → collapse) |
...
| Mouse report | `src/Surface.zig` `mouseReport` | `src/engine/mouse.zig` + `apprt/win32` wiring | template | SGR 1006 + X10 button press/release encode (not exposed by libghostty-vt); motion/wheel/other formats pending (#52) | done (host: encoder) |
...
- **Tier 1 — usable terminal**: first cut **landed** — clipboard copy/paste
  ...
  *encoding* is not exposed, so it is ported in `src/engine/mouse.zig`.)
- **Tier 2 — quality / parity**: the native backends still pending a Windows host
  (multi-surface runtime, scrollbar drawing — see the rows above and #43;
  **Direct3D 11** is now a native backend alongside OpenGL/WGL (#43); HarfBuzz
  shaping is now built from source and verified via `shape-proof`, #79). The
  **full `config/` system** (#49) is
  now ported: the Config row's package mirrors ghostty's `config/` (Config field
  surface, file_load, path, theme, conditional, command, formatter, string) with
  `config-file` includes, theme + light/dark conditional resolution, and the
  write-back formatter. (ghostty's `config/` carries ~14.5K LOC including the
  C/Wasm APIs, doc-comment codegen, and a several-hundred-entry iTerm2 theme
  catalog; whostty ports the system + a broad field surface and embeds a small
  built-in theme catalog, leaving the CLI-codegen-bound pieces to #53.)
