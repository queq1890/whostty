...
| Sprite glyphs | `src/font/sprite/` (`Box.zig`, `Canvas.zig`) | `src/font/sprite.zig` (used by `GlyphCache`) | port (slice) | block elements (U+2580–259F: halves/eighths/quadrants/shades) + braille (U+2800–28FF) rasterized procedurally into a cell-sized alpha bitmap, packed like a font glyph, font-independent (#76); box-drawing lines (U+2500–257F) + Powerline/Nerd Font follow-up | done (host: per-category pixels); GL (offscreen-proof: full block fills cell, quadrant a quarter) |
...
| Font (Freetype) | `src/font/main.zig` | `src/font/main.zig` | port | opt-in (`-Dfreetype`); `glyphIndex` (skip absent glyphs) added (#66); synthetic bold (outline embolden) + italic (outline shear) from the single regular face (#77); per-style families (`font-family-bold` etc.) deferred (#13/#14) | host (needs font); GL (offscreen-proof: bold adds ink) |
