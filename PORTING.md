...
| apprt (Windows) | `src/apprt/gtk/` | `src/apprt/win32/Window.zig` | template | done (window+WGL); **live window title** from OSC 0/2 (`Termio` captures `.window_title` → UI thread drains → `Window.setTitle`/`SetWindowTextW`, codepoint-bounded UTF-16 to avoid overflow); CSI 21t/22t/23t deferred (#89) | fresh tests |
