...
| Terminal IO | `src/termio.zig`, `src/termio/` | `src/termio.zig` | port | done + viewport scroll (#16) + VT response write-back (DSR/DA/DECRQM/ENQ) via a handler wrapping `ReadonlyHandler` (#81) + OSC 4/10/11/12 color queries answered from `terminal.colors` (#83); xtversion/size/kitty-query replies (#82) deferred | done (host) |
